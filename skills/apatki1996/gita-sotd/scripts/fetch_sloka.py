#!/usr/bin/env python3
"""
Fetch a Bhagavad Gita sloka from the vedicscriptures API.
Returns sloka of the day (deterministic by date) or a specific/random sloka.
"""
...
def fetch_sloka(chapter: int, verse: int) -> dict:
    """Fetch sloka data from the API."""
    url = f"{API_BASE}/slok/{chapter}/{verse}"
    try:
        with urlopen(url, timeout=10) as resp:
            return json.loads(resp.read().decode("utf-8"))
    except (URLError, HTTPError) as e:
        print(f"Error fetching sloka: {e}", file=sys.stderr)
        sys.exit(1)


def format_sloka(data: dict, translator: str = "prabhu", include_sanskrit: bool = True) -> str:
    """Format sloka for display."""
    lines = []
...
def main():
    parser = argparse.ArgumentParser(description="Fetch Bhagavad Gita sloka")
    parser.add_argument("--chapter", "-c", type=int, help="Chapter number (1-18)")
    parser.add_argument("--verse", "-v", type=int, help="Verse number")
    parser.add_argument("--daily", "-d", action="store_true", help="Get sloka of the day")
    parser.add_argument("--random", "-r", action="store_true", help="Get random sloka")
    parser.add_argument("--translator", "-t", default="prabhu",
    ...
    data = fetch_sloka(ch, vs)
    
    if args.json:
        print(json.dumps(data, ensure_ascii=False, indent=2))
    else:
        print(format_sloka(data, args.translator, not args.no_sanskrit))
